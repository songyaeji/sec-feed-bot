...
def build_cards(
    ...
    # 표지 1 + 뉴스 + 오늘의 CVE 1(상시) + (그 외 있을 때 1)
    total = 1 + len(top) + 1 + (1 if other_rest else 0)
    ...
    # '오늘의 CVE'는 항상 맨 마지막 장 — 0건이어도 빈 상태 카드로 발행 기간에
...
def _build_news(
    ...
    if po:
        pullout = _fill(fragments["pullout"],
